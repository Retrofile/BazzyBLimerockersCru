...
function go(els, opts, manual, fwd) {
    ...
    if (manual || !p.cyclePause) {
        ...
        if (opts.random) {
			...
			if (typeof(exifcaption)!="undefined") {
				tmpCap=exifcaption[opts.currSlide].split(" ");
				$("#caption").html(tmpCap[0]);
				currImage=$("#s1 img:nth-child("+(opts.currSlide+1)+")");
				w=currImage.width();
				h=currImage.height();
				wh=parseInt($(window).height())-150;
				if (wh>=470) h=450;
				h+=10;
				if (exceptionBrowser==false) {
					$(".nextPrev").animate({marginTop:h},"normal");
					$("#nav").animate({marginTop:h},"normal");
				}
			}
        }
        if (opts.pager)
...
function buildPager(els, opts) {
    ...
    $.each(els, function(i,o) {
        var $a = (typeof opts.pagerAnchorBuilder == 'function')
            ...
            : $('<a href="#" class="pageNumbers" title="'+exifcaption[i]+'">'+(i+1)+'</a>');
